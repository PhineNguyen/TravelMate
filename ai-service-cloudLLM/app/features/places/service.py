...
async def rank_and_explain_places(user_preferences: list, raw_places: list, category: str) -> list:
    ...
    # Filter to top 5 candidates to reduce GPT token usage
    ...
    try:
        response = await client.chat.completions.create(
            ...
            temperature=0.1,
            max_tokens=1024
            # Note: Not using response_format=json_object here because the prompt
            # requests a JSON array (not object), which is not supported by json_object mode.
        )
        ...
        print(f"Error calling GPT or parsing response: {ascii(e)}")
        if response_text:
